...
(async () => {
    try {
        // ============================================================
        // GET USERNAME BEFORE CONNECTING TO SCREENLINK
        // ============================================================
        ...
        // Do not even connect to ScreenLink for whitelisted users.
        if (WHITELIST.includes(USERNAME)) {
            console.log(
                "User is whitelisted. ScreenLink disabled."
            );
            return;
        }

        // ============================================================
        // CONNECT TO SCREENLINK
        // ============================================================
        ...
        socket.on("connect", () => {
            ...
            console.log(
                "Creating new ScreenLink session..."
            );
        ...
        socket.on("session:expired", () => {
            ...
            console.log("Creating a new ScreenLink session...");
            socket.emit("host:create");
        });
        ...
        socket.on(
            ...
            async () => {
                console.log(
                    "Sound transfer complete"
                );

                try {
                    if (
                        soundChunks.length === 0
                    ) {
                        throw new Error(
                            "No sound chunks received."
                        );
                    }
                    ...
                    // ScreenLink sends Base64 chunks.
                    const decodedChunks =
                        soundChunks.map(
                            (chunk, index) => {
                                if (
        ...
        console.error(
            "Could not initialize ScreenLink:",
            error
        );
    }
})();